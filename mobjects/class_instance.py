...
class ClassInstance(InstanceABC):
    ...
    def __init__(self, name: str, properties: list[Property] = None, size: float = 1.0, **kwargs):
        ...
        name_text.move_to(np.array([size * 0.5, -size * 0.5, 0]), LEFT)
        for i, t in enumerate(pr_texts):
            t.move_to(np.array([size * 0.5, -size * (1.5 + i), 0]), LEFT)

        for i in range(len(self.properties) + 3):
            start = np.array([0, -size * i, 0])
            end = np.array([w, -size * i, 0])
            l = Line(start, end, stroke_width=self.stroke_width)
            self.add(l)
        self.add(Line(np.array([0, 0, 0]), np.array([0, -h, 0]), stroke_width=self.stroke_width),
                 Line(np.array([w - size, -size, 0]), np.array([w - size, -h + size, 0]),
                      stroke_width=self.stroke_width),
                 Line(np.array([w, 0, 0]), np.array([w, -h, 0]), stroke_width=self.stroke_width))
